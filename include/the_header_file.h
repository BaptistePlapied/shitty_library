...
typedef struct {
    Vect p[3];       // Vertex positions
    Vect normal[3];  // Per-vertex normals for smooth shading
    Vect tangent[3]; // Per-vertex tangents for texture mapping
    Vect uv[3];      // Texture coordinates for each vertex
    Vect bezier[3];  // Bézier control points for cubic interpolation
